...
## Purdue's Bell cluster. This is set for use in multiple bash scripts to submit jobs via sbatch, will run DTW on 1 sim and 17 real profiles
...
# Each sbatch job will run DTW for 25 sims and 1 real profile

startsim =int(sys.argv[1])
sims2run = range(startsim, startsim+25)
...
for i in NPLD2run:
    ...
    for j in sims2run:
        ...
        if GoodFit == False:
            ...
            BadFit_list.append("N"+str(i+1)+"SIM"+sim_column_names[j])
        else:
            hist.axvline(x = XC, color = 'g', ls = '--')
            GoodFit_list.append("N"+str(i+1)+"SIM"+sim_column_names[j])
        
        fig.savefig("N"+str(i+1)+"SIM"+sim_column_names[j]+".png")
