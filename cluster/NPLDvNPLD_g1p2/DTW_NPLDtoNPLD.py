## Tunes depth profiles of the NPLD to each other using dynamic time warping algorithm
## Depth profiles should be .csv files located in ../data/NPLD
...
## Purdue's Bell cluster. This is set for use in multiple bash scripts to submit jobs via sbatch, will run DTW on 1 NPLD and 17 other NPLD profiles
...
# Each sbatch job will run DTW for 17 NPLD vs 1 NPLD

startNPLD1 =int(sys.argv[1])
NPLD12run = range(startNPLD1, startNPLD1+17)

NPLD2run = int(sys.argv[2])
NPLD2run = [NPLD2run]

## Import csv files -- NPLD1
csv_import = glob.glob("../../data/NPLD/*zoom.csv")
csv_import = sorted(csv_import, key=lambda x: int(x[-10]+x[-9]))
...
for i in NPLD2run:
    ...
    for j in NPLD12run:
        ...
        for a in range(1000):
            r=ar1(d2,v2,fit2) #this creates a random synthetic record with similar properties
            #r=ar1_nogauss(d2,v2)
            syn.append(r)
        ...
        if GoodFit == False:
            ...
            BadFit_list.append("N"+str(i+1)+"N"+str(j+1))
        else:
            hist.axvline(x = XC, color = 'g', ls = '--')
            GoodFit_list.append("N"+str(i+1)+"N"+str(j+1))
        
        fig.savefig("N"+str(i+1)+"N"+str(j+1)+".png")
